...
func TestMigrateUpgradesOriginalSchema(t *testing.T) {
	...
	if migrationsApplied != 10 {
		t.Fatalf("migration count=%d, want 10", migrationsApplied)
	}
}
...
func TestCustomerKeyRepairMigrationUpgradesRecorded003WithoutChecksumDrift(t *testing.T) {
	...
	if applied != 10 {
		t.Fatalf("migration count after idempotent repair = %d, want 10", applied)
	}
}
...
func TestCatalogKeyMigrationUpgradesPre005DatabaseWithoutChecksumDrift(t *testing.T) {
	...
	var applied int
	if err := s.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM schema_migrations`).Scan(&applied); err != nil || applied != 10 {
