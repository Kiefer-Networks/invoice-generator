...
func TestMigrateUpgradesOriginalSchema(t *testing.T) {
	...
	if migrationsApplied != 9 {
		t.Fatalf("migration count=%d, want 9", migrationsApplied)
	}
}
...
func TestCustomerKeyRepairMigrationUpgradesRecorded003WithoutChecksumDrift(t *testing.T) {
	...
	if applied != 9 {
		t.Fatalf("migration count after idempotent repair = %d, want 9", applied)
	}
}
...
func TestCatalogKeyMigrationUpgradesPre005DatabaseWithoutChecksumDrift(t *testing.T) {
	...
	var applied int
	if err := s.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM schema_migrations`).Scan(&applied); err != nil || applied != 9 {
