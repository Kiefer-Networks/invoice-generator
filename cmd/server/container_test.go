...
func TestContainerRuntime(t *testing.T) {
	...
	run("exec", id, "sh", "-c", `test "$(id -u)" = 65532 && test ! -e /usr/local/bin/browser.test && test ! -e /src && test ! -e /development-assets && test -z "$(find /run/secrets -type f 2>/dev/null)" && test -s /usr/share/doc/invoice-generator/licenses/go/LICENSE && test -s /usr/share/doc/invoice-generator/licenses/compiled-modules.txt && test -r /etc/fonts/local.conf && test "$(fc-match system-ui -f '%{family}')" = 'Liberation Sans' && test ! -w /usr/local/bin && test ! -w /etc && grep -q '^CapEff:.*0000000000000000$' /proc/1/status && grep -q '^NoNewPrivs:.*1$' /proc/1/status`)
