...
Source assessment performed 2026-09-10 and updated against commit `aa5322f206a0b536694bead4e0308bca0d75bbe2`. Every one of the 253 L1/L2 controls was compared with the normative OWASP ASVS 5.0.0 text from the pinned `v5.0.0` tag, then checked against the implementation locations and evidence named in its row. The assessment included fresh `go test ./...`, focused cookie and session-management tests, the real Chrome workflow, vet, staticcheck, golangci-lint, action/container policy, actionlint, module verification, govulncheck, focused snapshot/ZUGFeRD/backup/Paperless tests, and source/diff review.
...
| 3.3.1 | 1 | Pass: production cookies use `__Host-` names and `Secure` | `internal/auth/session.go; internal/auth/session_test.go; internal/web/server.go` | `TestCookieNamesSeparateSecureAndLoopbackDevelopment`; `TestSecurityHeadersAndAuthenticationFlow` | Application maintainer + operator |
...
| 3.3.3 | 2 | Pass: production session and OIDC transaction cookies use host-only `__Host-` names | `internal/auth/session.go; internal/auth/transaction.go; internal/auth/session_test.go` | `TestCookieNamesSeparateSecureAndLoopbackDevelopment`; `TestSecurityHeadersAndAuthenticationFlow` | Application maintainer + operator |
...
| 7.1.2 | 2 | Pass: five concurrent sessions are documented and an additional login is rejected atomically | `internal/auth/oidc.go; internal/store/auth.go; docs/container-deployment.md` | `TestManagerEnforcesConcurrentSessionLimitAndCanManageSessions`; `TestCreateSessionLimitedEnforcesMaximumAtomically` | Application maintainer |
...
| 7.4.2 | 1 | Operator pending: couple Pocket ID account disablement/deletion to immediate `server sessions revoke` or `revoke-all` and verify the runbook | `cmd/server/main.go; internal/store/auth.go; docs/container-deployment.md` | `TestRevokeSpecificSession`; `TestRevokeAllSessions`; exercise the production identity-offboarding procedure | Application maintainer + Pocket ID operator |
...
| 7.4.5 | 2 | Pass: administrators can revoke a specific opaque session ID or all sessions | `cmd/server/main.go; internal/store/auth.go; docs/container-deployment.md` | `TestRevokeSpecificSession`; `TestRevokeAllSessions` | Application maintainer |
...
| 7.5.2 | 2 | Pass: authenticated users can view, identify, and revoke current, other, or all active sessions | `internal/auth/oidc.go; internal/web/sessions.go; internal/web/templates/sessions.html` | `TestManagerEnforcesConcurrentSessionLimitAndCanManageSessions`; `TestSessionPageDoesNotExposeSecretsAndRevokesSpecificSession` | Application maintainer |
